...
export async function PATCH(request: Request) {
  ...
  if (!session || (session.user as any)?.role !== 'admin') {
    return NextResponse.json({ error: 'Solo admins pueden cancelar asignaciones' }, { status: 403 });
  }
  ...
  try {
    ...
    const idx = headerMap.get('CANTIDAD');
    if (idx === undefined) {
      return NextResponse.json({ error: 'Columna CANTIDAD no encontrada en VENTAS' }, { status: 500 });
    }

    await batchUpdateCells(spreadsheetId, [
      { range: `VENTAS!${indexToColumnLetter(idx)}${rowIndex}`, value: 0 },
    ]);

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error PATCH /api/ventas/cancelar:', error);
